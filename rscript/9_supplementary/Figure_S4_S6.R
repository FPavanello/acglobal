...
global$country2[global$country == "Germany"] <- "Germany"
...
# Test LM
reg <- lm(estimate*100 ~ as.numeric(country), data = all); summary(reg) # p-value 0.017
reg <- summary(reg)
...
exp <- global %>% 
  dplyr::group_by(country2) %>% 
  dplyr::summarise(mean = weighted.mean(total_exp_usd_2011, weight, na.rm = TRUE)) %>% # expenditure per capita
  dplyr::arrange(mean)
...
# Test LM
reg <- lm(estimate*100 ~ as.numeric(country), data = all); summary(reg) # p-value 0.028
reg <- summary(reg)
pval <- round(reg$coefficients[2,4], 3)
...
# https://data.worldbank.org/indicator/NY.GDP.PCAP.PP.KD (OECD-EU = 62,127, OECD-NONEU = 52,959)
order <- c("Africa", "Pakistan", "India", "Indonesia", "Brazil", "China", "Mexico", "Argentina", "Italy", 
           "OECD-NonEU", "OECD-EU", "Germany", "USA") 

# Factor order
all$country <- factor(all$country, levels = c(order))

# Test LM
reg <- lm(estimate*100 ~ as.numeric(country), data = all); summary(reg) # p-value 0.008
reg <- summary(reg)
pval <- round(reg$coefficients[2,4], 3)
...
ac_country <- all %>% 
  ...
  theme(panel.background=element_blank(),
        ...
        #                    axis.title.x = element_blank(),
        axis.ticks=element_blank()) +
  annotate("text", x = "Germany", family = "mono", y=90, label = paste0("P-value = ", pval, sep = ""), fontface = "bold")
