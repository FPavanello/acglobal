...
load(paste0(stub, "repo/interm/oecdeu_dmcf.Rdata"))

HH_Europe = HH_Europe[!is.na(HH_Europe$ln_ely_q),]
...
gadm_1 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_FRA_1_sp.rds")))
gadm_2 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_SWE_1_sp.rds")))
gadm_4 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_ESP_1_sp.rds")))
gadm_5 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_NLD_1_sp.rds")))

gadm <- bind_rows(gadm_1, gadm_2, gadm_4 , gadm_5)
...
pop_features <- filter(pop_features, REGION %in% c("FRA", "SWE", "ESP", "NLD"))
...
for (country in 1:4){
  ...
  pop_features_edu <-   pop_features_edu[grep(c("FRA", "SWE", "ESP", "NLD")[country], names(pop_features_edu))]  
...
ss <- filter(ss, ISO3 %in% c("FRA", "SWE", "ESP", "NLD"))
...
national_summary_cons_bycountry <- output_impact_ac %>% mutate(country=data_c_sp$country) %>% group_by(country) %>% 
  dplyr::summarise_all(mean, na.rm=T) %>%
  pivot_longer(cols = 2:9, names_to = c('ssp', 'year'), names_sep = ".") %>%
  mutate(ssp=rep(rep(c("SSP245","SSP585"), each=4), length(unique(data_c_sp$country))), year=rep(seq(2020, 2050, 10), 2*length(unique(data_c_sp$country))))

national_summary_cons_bycountry = group_by(national_summary_cons_bycountry, country, ssp) %>% mutate(value=ifelse(is.na(value), min(value, na.rm=T), value))
...
national_summary_total_bycountry = group_by(national_summary_total_bycountry, country, ssp) %>% mutate(value=ifelse(is.na(value), min(value, na.rm=T), value))
...
write.csv(national_summary_cons_totelectr,  paste0(stub, "repo/interm/projections", countryiso3, "_national_ely_consumption_bycountry_glomod.csv"))
