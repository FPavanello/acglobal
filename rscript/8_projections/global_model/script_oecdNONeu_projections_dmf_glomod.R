...
countryname <- "OECD_NONEU"
countryiso3 <- "OECD_NONEU"
...
gadm_1 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_JPN_1_sp.rds")))
gadm_2 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_CAN_1_sp.rds")))
gadm_3 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_AUS_1_sp.rds")))

gadm <- bind_rows(gadm_1, gadm_2, gadm_3)
...
pop_features <- filter(pop_features, REGION %in% c("CAN", "AUS", "JPN"))
...
pop_features_gender <- pop_features_gender %>% group_by(REGION, SCENARIO) %>% mutate(value=growth_rate(value) / 100)
...
custom_shape$country <- gadm$GID_0

custom_shape <- merge(custom_shape, pop_features_gender, by.x="country", by.y="REGION")
...
pop_features_age <- dplyr::group_by(pop_features_age, REGION, SCENARIO) %>% dplyr::mutate((X2010=X2010*VARIABLE)/sum(X2010), (X2020=X2020*VARIABLE)/sum(X2020), (X2030=X2030*VARIABLE)/sum(X2030), (X2040=X2040*VARIABLE)/sum(X2040), (X2050=X2050*VARIABLE)/sum(X2050))

pop_features_age <- dplyr::select(pop_features_age, c(1, 2, 3, 14:18))

colnames(pop_features_age)[4:8] <- paste0("X", seq(2010, 2050, 10))

pop_features_age <- pop_features_age %>% group_by(SCENARIO, REGION) %>% dplyr::summarise(X2010=sum(X2010*VARIABLE), X2020=sum(X2020*VARIABLE), X2030=sum(X2030*VARIABLE), X2040=sum(X2040*VARIABLE), X2050=sum(X2050*VARIABLE))
...
pop_features_age <- pop_features_age %>% group_by(SCENARIO, REGION) %>% mutate(value=growth_rate(value) / 100)
...
custom_shape <- merge(custom_shape, pop_features_age, by.x="country", by.y="REGION")

# Edu (national SSPs)

pop_features_edu <-filter(pop_features, grepl("Education",VARIABLE))
pop_features_edu <- dplyr::select(pop_features_edu, c(2:4, 5:14))
pop_features_edu$VARIABLE <- gsub('.*\\|', "", pop_features_edu$VARIABLE)

pop_features_edu <- group_by(pop_features_edu, REGION, SCENARIO) %>% dplyr::mutate((X2010=X2010)/sum(X2010), (X2020=X2020)/sum(X2020), (X2030=X2030)/sum(X2030), (X2040=X2040)/sum(X2040), (X2050=X2050)/sum(X2050))

pop_features_edu <- dplyr::select(pop_features_edu, c(1, 2, 3, 14:18))

colnames(pop_features_edu)[4:8] <- paste0("X", seq(2010, 2050, 10))

pop_features_edu <- group_by(pop_features_edu, SCENARIO, REGION, VARIABLE) %>% dplyr::summarise_all(., "sum")
...
pop_features_edu <- dplyr::select(pop_features_edu, 1:8)
pop_features_edu$REGION<-as.character(pop_features_edu$REGION)

pop_features_edu <- split(pop_features_edu, list(pop_features_edu$SCENARIO, pop_features_edu$REGION))

for (i in 1:length(pop_features_edu)){
  
  pop_features_edu[[i]]$SCENARIO<- NULL
  pop_features_edu[[i]]$REGION<- NULL
...
pop_features_edu_bk <-pop_features_edu

out <- list()

for (country in 1:3){
  
  pop_features_edu <-   pop_features_edu[grep(c("JPN", "AUS", "CAN")[country], names(pop_features_edu))]  
  
  for (scen in 1:5){
    
    edu_ssp1 <- pop_features_edu[[scen]]
    row.names(edu_ssp1) <- 1:5
    edu_ssp1_hhs <- data.frame(edu_2010=as.numeric(as.character(data_c_split[[country]]$edu_head_2)))
  ...
  out[[country]] <- pop_features_edu
  
  pop_features_edu <- pop_features_edu_bk
  
}

data_c <- data_c[!is.na(data_c$edu_head_2),]

pop_features_edu <- bind_rows(out)
...
# 
# housing_index_lab_s1[c(2:5),2] <- 0
# 
# 
# for (i in 2:5){
#   for (j in 3:6){
...
# 
#       housing_index_lab_s1[i,j-1] <- housing_index_lab_s1[i,j-1] - rate_improvement_housing_1
# 
#     }
# 
#   }}
...
# 
# housing_index_lab_s2[c(2:5),2] <- 0
# 
# 
# for (i in 2:5){
#   for (j in 3:6){
...
#     } else{
# 
#       housing_index_lab_s2[i,j] <- housing_index_lab_s2[i-1,j] + rate_improvement_housing_2
# 
#       housing_index_lab_s2[i,j-1] <- housing_index_lab_s2[i,j-1] - rate_improvement_housing_2
# 
#     }
# 
#   }}
...
# 
# data_c <- data_c[!is.na(data_c$housing_index_lab),]
# 
...
ss <- filter(ss, ISO3 %in% c("JPN", "AUS", "CAN"))
...
cdd_hist_cmip6 <- readRDS(paste0(stub, "data/projections/climate/processed/", CDD_type, "/", "OECD", "_CDD_model_ensemble_median_hist.rds"))

cdd_245_cmip6 <- readRDS(paste0(stub, "data/projections/climate/processed/", CDD_type, "/", "OECD", "_CDD_model_ensemble_median_rcp45_ssp2.rds"))

cdd_585_cmip6 <- readRDS(paste0(stub, "data/projections/climate/processed/", CDD_type, "/", "OECD", "_CDD_model_ensemble_median_rcp85_ssp5.rds"))

#

hdd_hist_cmip6 <- readRDS(paste0(stub, "data/projections/climate/processed/", HDD_type, "/", "OECD", "_HDD_model_ensemble_median_hist.rds"))

hdd_245_cmip6 <- readRDS(paste0(stub, "data/projections/climate/processed/", HDD_type, "/", "OECD", "_HDD_model_ensemble_median_rcp45_ssp2.rds"))

hdd_585_cmip6 <- readRDS(paste0(stub, "data/projections/climate/processed/", HDD_type, "/", "OECD", "_HDD_model_ensemble_median_rcp85_ssp5.rds"))


#

cdd_hist_cmip6 <- dplyr::group_by(cdd_hist_cmip6, region) %>%  dplyr::select(contains("mean")) %>% dplyr::summarise_all(.funs="mean", na.rm=T)
cdd_245_cmip6 <- dplyr::group_by(cdd_245_cmip6, region) %>%  dplyr::select(contains("mean")) %>% dplyr::summarise_all(.funs="mean", na.rm=T)
cdd_585_cmip6 <- dplyr::group_by(cdd_585_cmip6, region) %>%  dplyr::select(contains("mean")) %>% dplyr::summarise_all(.funs="mean", na.rm=T)

hdd_hist_cmip6 <- dplyr::group_by(hdd_hist_cmip6, region) %>%  dplyr::select(contains("mean")) %>% dplyr::summarise_all(.funs="mean", na.rm=T)
hdd_245_cmip6 <- dplyr::group_by(hdd_245_cmip6, region) %>%  dplyr::select(contains("mean")) %>% dplyr::summarise_all(.funs="mean", na.rm=T)
hdd_585_cmip6 <- dplyr::group_by(hdd_585_cmip6, region) %>%  dplyr::select(contains("mean")) %>% dplyr::summarise_all(.funs="mean", na.rm=T)
...
geo_bk_2 <- data_c_sp$geometry
data_c_sp$geometry <- NULL
...
#   scale_colour_npg()
# 
# ggsave(paste0(stub, "repo/interm/plot_gdp_1_", countryiso3, ".png"), plot_gdp_1)
# 
...
# 
# ggsave(paste0(stub, "repo/interm/plot_gdp_2_", countryiso3, ".png"), plot_gdp_2)
# 
# #
# 
...
#   scale_colour_npg()
# 
# ggsave(paste0(stub, "repo/interm/plot_pop_1_", countryiso3, ".png"), plot_pop_1)
# 
...
#   facet_wrap(~ state, ncol=10)
# 
# ggsave(paste0(stub, "repo/interm/plot_pop_2_", countryiso3, ".png"), plot_pop_2)
# 
# #
...
#   scale_colour_npg()
# 
# ggsave(paste0(stub, "repo/interm/plot_gdp_capita_1_", countryiso3, ".png"), plot_gdp_capita_1)
# 
# 
...
#   facet_wrap(~ state, ncol=10)
# 
# ggsave(paste0(stub, "repo/interm/plot_gdp_capita_2_", countryiso3, ".png"), plot_gdp_capita_2)
# 
# #
# 
...
#   scale_colour_npg()
# 
# ggsave(paste0(stub, "repo/interm/plot_urb_1_", countryiso3, ".png"), plot_urb_1)
# 
# 
...
#   facet_wrap(~ state, ncol=10)
# 
# ggsave(paste0(stub, "repo/interm/plot_urb_2_", countryiso3, ".png"), plot_urb_2)
# 
# 
# ##
# 
...
#   scale_fill_npg()
# 
# ggsave(paste0(stub, "repo/interm/housing_index_lab_improvement_s1_", countryiso3, ".png"), housing_index_lab_plot)
# 
# #
# 
...
#   scale_fill_npg()
# 
# ggsave(paste0(stub, "repo/interm/housing_index_lab_improvement_s2_", countryiso3, ".png"), housing_index_lab_plot)
...
load(paste0(stub, "repo/interm/global_wgt_dmcf.Rdata")); data_c_sp$country = data_c_sp$country.x
...
ggsave(paste0(stub, "repo/interm/line_country_ac_", countryiso3, ".png"), line_country_ac)
...
ggsave(paste0(stub, "repo/interm/line_region_ac_", countryiso3, ".png"), line_region_ac)

gadm <- gadm %>% dplyr::select(geometry, NAME_1)

regional_summary_ac$NAME_1 <- regional_summary_ac$state
...
#ggsave(paste0(stub, "repo/interm/map_ac_", countryiso3, ".png"), map_ac, scale=1.75)
...
#ggsave(paste0(paste0(stub, "repo/interm/historical_decomp.png")))
...
weighted_mean_na <- function(x, w, ..., na.rm = FALSE){
...
baseline_hist <- weighted_mean_na(exp(baseline_hist), data_c$weight, na.rm=T)
decomp_ac <- weighted_mean_na(exp(decomp_ac), data_c_sp[,paste0("weight_", year, "_", rcp, "_", tolower(ssp))], na.rm=T)
decomp_exp <- weighted_mean_na(exp(decomp_exp), data_c_sp[,paste0("weight_", year, "_", rcp, "_", tolower(ssp))], na.rm=T)
decomp_cdds <- weighted_mean_na(exp(decomp_cdds), data_c_sp[,paste0("weight_", year, "_", rcp, "_", tolower(ssp))], na.rm=T)
decomp_hdds <- weighted_mean_na(exp(decomp_hdds), data_c_sp[,paste0("weight_", year, "_", rcp, "_", tolower(ssp))], na.rm=T)
decomp_urb <- weighted_mean_na(exp(decomp_urb), data_c_sp[,paste0("weight_", year, "_", rcp, "_", tolower(ssp))], na.rm=T)
decomp_total <- weighted_mean_na(exp(total), data_c_sp[,paste0("weight_", year, "_", rcp, "_", tolower(ssp))], na.rm=T)
...
ggsave(paste0(stub, "repo/interm/line_country_ely_q_", countryiso3, ".png"), line_country_ely_q)
...
#   scale_colour_npg()
#
# ggsave(paste0(stub, "repo/interm/line_country_ely_q_share_", countryiso3, ".png"), line_country_ely_q_share, scale=1.5)
...
write.csv(national_summary_ac, paste0(stub, "repo/interm/projections", countryiso3, "_national_ac_penetration_glomod.csv"))
write.csv(national_summary_cons,  paste0(stub, "repo/interm/projections", countryiso3, "_national_ac_consumption_glomod.csv"))
write.csv(national_summary_total,  paste0(stub, "repo/interm/projections", countryiso3, "_national_ac_consumption_total_glomod.csv"))
...
write.csv(national_summary_total_bycountry,  paste0(stub, "repo/interm/projections", countryiso3, "_national_ac_consumption_total_bycountry_glomod.csv"))
...
write.csv(national_summary_cons_totelectr,  paste0(stub, "repo/interm/projections", countryiso3, "_national_ely_consumption_glomod.csv"))
...
write.csv(national_summary_cons_totelectr_bycountry,  paste0(stub, "repo/interm/projections", countryiso3, "_national_ely_consumption_bycountry_glomod.csv"))
