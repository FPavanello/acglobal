...
gadm <- read_sf(paste0(stub, "data/shapefiles/Indonesia/Adm_level_1_provinces/IDN_adm1.shp"))
...
gadm_0 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_IDN_0_sp.rds")))
gadm_1 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_IDN_1_sp.rds")))
...
# extract the data
gdp_extracted <- exactextractr::exact_extract(gdp_ssps_data, custom_shape, fun="sum")
pop_extracted <- exactextractr::exact_extract(pop_ssps_data, custom_shape, fun="sum")
...
urban_extracted <- exactextractr::exact_extract(urban_ssps_data, custom_shape, fun="weighted_mean", weights=weight_raster)
...
gdp_capita <- as.data.frame(custom_shape[,c(1:50)] / custom_shape[,c(51:100)])
...
for (i in 1:nrow(gdp_capita)){
  gdp_capita_growth[i,c(1:10)] <- growth_rate(as.numeric(gdp_capita[i,c(1:10)])) / 10
  gdp_capita_growth[i,c(11:20)] <- growth_rate(as.numeric(gdp_capita[i,c(11:20)])) / 10
  gdp_capita_growth[i,c(21:30)] <- growth_rate(as.numeric(gdp_capita[i,c(21:30)])) / 10
  gdp_capita_growth[i,c(31:40)] <- growth_rate(as.numeric(gdp_capita[i,c(31:40)])) / 10
  gdp_capita_growth[i,c(41:50)] <- growth_rate(as.numeric(gdp_capita[i,c(41:50)])) / 10
}
...
for (ssp in c("SSP1", "SSP2", "SSP3", "SSP4", "SSP5")){
  ...
  for (year in seq(2020, 2050, 10)){
    ...
    data_c_sp[,paste0("exp_cap_usd_", ssp, "_", year)] <- data_c_sp[,paste0("exp_cap_usd_", ssp, "_", year-10)] * (1+((data_c_sp[,paste0("sum.GDP_capita_yearly_avg_growth_", ssp, "_", year)]/100)))^10
...
cmip6_merged$state3 <- cmip6_merged$adm1
...
future_ac_adoption$state3 <- data_c_sp$state2.x

regional_summary_ac <- future_ac_adoption %>%
  group_by(state3) %>%
  dplyr::summarise_all(mean, na.rm=T) %>%
  pivot_longer(cols = 2:9, names_to = c('ssp', 'year'), names_sep = ".") %>%
  mutate(ssp=rep(rep(c("SSP245", "SSP585"), each=4), length(unique(data_c_sp$state2.x))), year=rep(rep(seq(2020, 2050, 10), 2), length(unique(data_c_sp$state2.x))))
...
regional_summary_ac <- merge(regional_summary_ac, gadm_1, by.x="state3", by.y="NAME_1")
...
# multiply by average consumption due to A
C
...
output_impact_ac$ely_p  = ifelse(output_impact_ac$ely_p > 0.172572, 0.172572, output_impact_ac$ely_p)
