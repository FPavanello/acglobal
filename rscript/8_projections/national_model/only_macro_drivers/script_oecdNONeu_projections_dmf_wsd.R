...
for (year in seq(2020, 2050, 10)){
  ...
  for (scen in c("rcp45_ssp2", "rcp85_ssp5")){
    ...
    data_c_sp[,paste0("mean_CDD_", year, "_", scen)] <- data_c_sp[,paste0("delta_", scen, "_", year)] + (data_c_sp$curr_CDD18_db * 100)
...
for (year in seq(2020, 2050, 10)){
  ...
  for (scen in c("rcp45_ssp2", "rcp85_ssp5")){
    ...
    data_c_sp[,paste0("mean_HDD_", year, "_", scen)] <- data_c_sp[,paste0("delta_", scen, "_", year)] + (data_c_sp$curr_HDD18_db * 100)
...
orig_data <- HH_NonEurope[fixest::obs(reg_ac),]
...
for (ssp in c("SSP2", "SSP5")){
  ...
  orig_data_bk <- HH_NonEurope[fixest::obs(reg_ac),]
...
orig_data <- HH_NonEurope[obs(model3),]
...
write.csv(as.data.frame(regional_summary_ac),  paste0(stub, "repo/interm/projections", countryiso3, "_regional_ac_penetration_wsd.csv"))
# write.csv(regional_summary_cons,  paste0(stub, "repo/interm/projections", countryiso3, "_regional_consumption_wsd.csv"))
# write.csv(regional_summary_total,  paste0(stub, "repo/interm/projections", countryiso3, "_regional_consumption_total_wsd.csv"))
...
write.csv(national_summary_cons_totelectr,  paste0(stub, "repo/interm/projections", countryiso3, "_national_ely_consumption_wsd.csv"))