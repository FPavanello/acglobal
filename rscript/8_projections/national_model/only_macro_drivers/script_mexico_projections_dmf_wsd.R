...
load(paste0(stub, "repo/interm/mex_dmcf.RData"))
...
gadm <- read_sf(paste0(stub, "data/shapefiles/Mexico/Adm_level_2_municipalities/Muni_2012gw.shp"))
...
# # differentiate growth rates for each quintile
# 
# proj = read_rds(paste0(stub, "data/economic/projections.rds"))
# proj = filter(proj, Var1 == countryiso3)
...
cmip6_merged$state_district <- paste0(cmip6_merged$state2, " - ", cmip6_merged$district2)
...
orig_data <- HH_Mexico[fixest::obs(reg_ac),]
...
for (ssp in c("SSP2", "SSP5")){
  ...
  orig_data_bk <- HH_Mexico[fixest::obs(reg_ac),]
...
regional_summary_ac$state3 <- as.character(regional_summary_ac$state3)

regional_summary_ac$state3[regional_summary_ac$state3=="Coahuila de Zaragoza"] <- "Coahuila"
regional_summary_ac$state3[regional_summary_ac$state3=="Michoac?n de Ocampo"] <- "Michoac?n"
regional_summary_ac$state3[regional_summary_ac$state3=="Quer?taro Arteaga"] <- "Quer?taro"
regional_summary_ac$state3[regional_summary_ac$state3=="Veracruz de Ignacio de la Llave"] <- "Veracruz"
...
orig_data <- HH_Mexico[obs(model3),]
...
for (ssp in c("SSP2", "SSP5")){
  ...
  for (year in seq(2020, 2050, 10)){
    ...
    #
    projected <- predict(model3, newdata=orig_data)
