...
load(paste0(stub, "results/regressions/for_projections//arg_dmcf.RData"))
...
gadm_0 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_ARG_0_sp.rds")))
gadm_1 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_ARG_1_sp.rds")))
...
data_c$housing_index <- data_c$housing_index_lab
...
housing_index_lab_s1[1,c(2:4)] <-  table(data_c$housing_index) / sum(table(data_c$housing_index))
...
housing_index_lab_s1[2,3] <- housing_index_lab_s1[2,3] + housing_index_lab_s1[2,2]
housing_index_lab_s1[2,2] <- 0
...
housing_index_lab_s2[1,c(2:4)] <-  table(data_c$housing_index) / sum(table(data_c$housing_index))
...
orig_data <- HH_Argentina[fixest::obs(reg_ac),]
...
for (ssp in c("SSP2", "SSP5")){
  ...
  orig_data_bk <- HH_Argentina[fixest::obs(reg_ac),]
...
orig_data <- HH_Argentina[obs(model3),]
...
output_impact_ac$ely_p  = ifelse(output_impact_ac$ely_p >  0.77310, 0.77310, output_impact_ac$ely_p)
