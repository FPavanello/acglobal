...
load(paste0(stub, "repo/interm/usa_dmcf.RData"))
...
gadm <- read_sf(paste0(stub, "data/shapefiles/United States/gadm40_USA_1.shp"))
ext_usa <- extent(gadm)
ext_usa[1] <- -168
ext_usa[2] <- -66
...
gadm_0 <- st_as_sf(raster::getData('GADM', country=countryiso3, level=0))
gadm_1 <- st_as_sf(raster::getData('GADM', country=countryiso3, level=1))
...
for (i in 1:5){
  ...
  pop_ssps_data[[i]] <- crop(pop_ssps_data[[i]], ext_usa)
...
urban_ssps_data <- lapply(urban_ssps_data, crop, ext_usa)
...
cdd_hist_cmip6 <- readRDS(paste0(stub, "data/projections/climate/processed/", CDD_type, "/", countryiso3, "_CDD_model_ensemble_median_hist_ahs.rds"))

cdd_245_cmip6 <-  readRDS(paste0(stub, "data/projections/climate/processed/", CDD_type, "/", countryiso3, "_CDD_model_ensemble_median_rcp45_ssp2_ahs.rds"))

cdd_585_cmip6 <-  readRDS(paste0(stub, "data/projections/climate/processed/", CDD_type, "/", countryiso3, "_CDD_model_ensemble_median_rcp85_ssp5_ahs.rds"))

#

hdd_hist_cmip6 <-  readRDS(paste0(stub, "data/projections/climate/processed/", HDD_type, "/", countryiso3, "_HDD_model_ensemble_median_hist_ahs.rds"))

hdd_245_cmip6 <- readRDS(paste0(stub, "data/projections/climate/processed/", HDD_type, "/", countryiso3, "_HDD_model_ensemble_median_rcp45_ssp2_ahs.rds"))

hdd_585_cmip6 <-  readRDS(paste0(stub, "data/projections/climate/processed/", HDD_type, "/", countryiso3, "_HDD_model_ensemble_median_rcp85_ssp5_ahs.rds"))
...
data_c_sp <- merge(data_c_sp, cmip6_merged, by.x="adm2", by.y="state")
...
for (ssp in c("SSP2", "SSP5")){
  ...
  orig_data_bk <- HH_USA[obs(reg_ac),]
...
bb <- bind_cols(data_c, future_ac_adoption, data_c_sp %>% dplyr::select(starts_with("weight_")))

bb <- bb %>% mutate(tercile=ntile(total_exp_usd_2011, 3))

dplyr::summarise(bb, ac=weighted.mean(as.numeric(as.character(SSP2.2050)), weight_2050_rcp45_ssp2, na.rm=T))

bb %>% dplyr::summarise(ac=weighted.mean(as.numeric(as.character(ac)), weight, na.rm=T))

bb %>% group_by(tercile) %>% dplyr::summarise(ac=weighted.mean(as.numeric(as.character(ac)), weight, na.rm=T))

bb %>% group_by(tercile) %>% dplyr::summarise(ac=weighted.mean(as.numeric(as.character(SSP2.2050)), weight_2050_rcp45_ssp2, na.rm=T))
...
metrics2_n$type <- ifelse(metrics2_n$var=="urban", "Urbanisation",  metrics2_n$type)
