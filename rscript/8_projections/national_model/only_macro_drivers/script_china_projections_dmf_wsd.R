...
load(paste0(stub, "results/regressions/for_projections/chn_dmcf.RData"))
...
gadm <- read_sf(paste0(stub, "data/shapefiles/China/gadm40_CHN_1.shp"))
...
gadm_0 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_CHN_0_sp.rds")))
gadm_1 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_CHN_1_sp.rds")))
...
data_c$housing_index_lab <- data_c$housing_index
...
housing_index_lab_s1[1,c(2:6)] <-  table(data_c$housing_index_lab) / sum(table(data_c$housing_index_lab))
...
housing_index_lab_s2[1,c(2:6)] <-  table(data_c$housing_index_lab) / sum(table(data_c$housing_index_lab))
...
custom_shape$state3 <- gadm$NAME_1

data_c_sp <- merge(data_c_sp, custom_shape, by.x="NAME_1.y", by.y="state3")
...
data_c_sp <- merge(data_c_sp, cmip6_merged, by.x="state3", by.y="state")
...
orig_data <- HH_China[fixest::obs(reg_ac),]
...
for (ssp in c("SSP2", "SSP5")){
  ...
  orig_data_bk <- HH_China[fixest::obs(reg_ac),]
...
ely_formula <- "ln_ely_q ~ ac + curr_CDD18_db + ln_total_exp_usd_2011 + curr_HDD18_db + urban_sh + n_members + ownership_d + edu_head_2 + sh_under16 + housing_index_lab + age_head + sex_head"
...
distr_country_ely_q <- ggplot(output_impact_ac2) +   geom_density(aes(x = value, y=..density.., group=year, colour=as.factor(year)), adjust=.75)+  geom_text(aes(x = mean, y=0.00325, label=year, group=year, colour=as.factor(year))) + facet_wrap(.~ssp, nrow=1)+
  xlim(c(0, 2500))+
  geom_vline(aes(xintercept = mean, group=year, colour=as.factor(year)), linetype="dashed", size=.5, alpha=0.6)+
  ylab("Density")+
...
ggsave(paste0(stub, "repo/interm/distr_country_ely_q_", countryiso3, ".png"), distr_country_ely_q, height = 5, width=8)
