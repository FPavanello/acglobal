...
load(paste0(stub, "results/regressions/for_projections/deu_dmcf.RData"))
...
gadm <- read_sf(paste0(stub, "data/shapefiles/Germany/gadm40_DEU_1.shp"))
...
gadm_0 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_DEU_0_sp.rds")))
gadm_1 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_DEU_1_sp.rds")))
...
orig_data <- HH_Germany[fixest::obs(reg_ac),]
...
library(imputeTS)
data_c_sp <- na_mean(data_c_sp, option = "median")
...
for (ssp in c("SSP2", "SSP5")){
  ...
  orig_data_bk <- HH_Germany[fixest::obs(reg_ac),]
  ...
  for (year in seq(2020, 2050, 10)){
    ...
    # projected <- predict(reg_ac, type="response")
    # projected <- ifelse(as.numeric(projected)>0.5, 1, 0)
    # summary(projected)
