...
gadm <- read_sf(paste0(stub, "data/shapefiles/brazil/Adm_level_1_states/BRA_adm1.shp"))
...
gadm_0 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_BRA_0_sp.rds")))
gadm_1 <- st_as_sf(readRDS(paste0(stub, "data/projections/new_data_jan_2022/gadm36_BRA_1_sp.rds")))
...
cdd_245_cmip6$country <- NULL
cdd_245_cmip6$state <- NULL
cdd_585_cmip6$country <- NULL
cdd_585_cmip6$state <- NULL
hdd_hist_cmip6$country <- NULL
hdd_hist_cmip6$state <- NULL
hdd_245_cmip6$country <- NULL
hdd_245_cmip6$state <- NULL
hdd_585_cmip6$country <- NULL
hdd_585_cmip6$state <- NULL

cmip6_merged <- bind_cols(cdd_hist_cmip6, cdd_245_cmip6, cdd_585_cmip6, hdd_hist_cmip6, hdd_245_cmip6, hdd_585_cmip6)

data_c_sp$geometry.y = NULL

data_c_sp <- merge(data_c_sp, cmip6_merged, by.x="NAME_1.y", by.y="state")

####################
# calibrate CDDs to historical / survey year CDDs to ensure consistency (for 2nd stage prediction only)

geo_bk_2 <- data_c_sp$geometry.x
