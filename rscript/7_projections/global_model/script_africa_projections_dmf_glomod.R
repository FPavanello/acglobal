...
load(paste0(stub, "results/regressions/for_projections/afr_dmcf.RData"))
...
gadm$ID_0 <- ifelse(is.na(gadm$ID_0), gadm$GID_0, gadm$ID_0)
gadm$GID_0 <- ifelse(is.na(gadm$GID_0), gadm$ID_0, gadm$GID_0)
...
pop_features <- pop_features %>% group_by(SCENARIO, REGION, VARIABLE) %>%  dplyr::mutate_at(vars(contains('X')), funs(median))
...
pop_features_gender <- pop_features_gender %>% group_by(REGION, SCENARIO) %>% dplyr::mutate(value=growth_rate(value) / 100)
...
custom_shape$country <- countrycode::countrycode(gadm$COUNTRY, 'country.name', 'iso3c')
...
pop_features_age <- pop_features_age %>% group_by(SCENARIO, REGION) %>% dplyr::mutate(value=growth_rate(value) / 100)
...
pop_features_edu <-   pop_features_edu_bk[grep(c("NGA", "GHA", "MWI", "NER", "BFA", "KEN")[country], names(pop_features_edu_bk))]  
...
cmip6_merged <- Reduce(function(dtf1, dtf2) merge(dtf1, dtf2, by = c("country", "state"), all = TRUE),
                       list(cdd_hist_cmip6, cdd_245_cmip6, cdd_585_cmip6, hdd_hist_cmip6, hdd_245_cmip6, hdd_585_cmip6))

data_c_sp <- merge(data_c_sp, cmip6_merged, by.x=c("country.x", "NAME_1.y"), by.y=c("country", "state"), all.x=T)

data_c_sp$country <- data_c_sp$country.x
data_c_sp$country.x <- NULL
...
# custom_shape_plotting$state <- gadm$NAME_1
# 
...
#   geom_line(aes(x=year, y=value/1e9, colour=SSP, group=SSP))+
#   scale_colour_npg()+
#   ylab("Gross GDP projection, billion USD")
# 
...
#   geom_line(aes(x=year, y=value/1e9, colour=SSP, group=SSP))+
#   scale_colour_npg()+
#   ylab("Total population, billion people")
# 
...
for (ssp in c("SSP2", "SSP5")){
  ...
  for (year in seq(2020, 2050, 10)){
    ...
    orig_data$housing_index_lab = as.factor(data_c_sp[,paste0("housing_index_lab_", year, "_s1")])
  	...
  	orig_data$country = data_c$country
...
ely_formula <- "ln_ely_q ~ as.factor(ac) + curr_CDD18_db +  ln_total_exp_usd_2011 +  urban_sh + n_members + as.factor(ownership_d) + edu_head_2 +   housing_index_lab + age_head + sex_head"
...
#   dplyr::mutate(ssp=rep(c("SSP1", "SSP2", "SSP3", "SSP4", "SSP5"), each=4), year=rep(seq(2020, 2050, 10), 5))
#
...
pop_long <- custom_shape %>% group_by(country) %>% filter(country!="TZA") %>%  dplyr::select(starts_with("sum.POP")) %>%  dplyr::summarise_at(vars(-country), "sum") %>% pivot_longer(2:50)
pop_long$name <- gsub("sum.POP_", "", pop_long$name )
pop_long$ssp <- substr(pop_long$name, 1, 4)
pop_long$year <- substr(pop_long$name, 6, 9)
pop_long <- filter(pop_long, year>2010 & year<=2050)
...
output_impact_ac2 <- output_impact_ac %>%
  pivot_longer(cols = 1:8, names_to = c('ssp', 'year'), names_sep = ".") %>%
  dplyr::mutate(ssp=rep(rep(c("SSP245","SSP585"), each=4), nrow(output_impact_ac)), year=rep(rep(seq(2020, 2050, 10), 2), nrow(output_impact_ac)))

output_impact_ac2 <- output_impact_ac2 %>% group_by(ssp, year) %>% dplyr::mutate(mean=median(value, na.rm=T))
