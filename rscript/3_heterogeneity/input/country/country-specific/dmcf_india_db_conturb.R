...
ely_formula_ind <- ln_ely_q ~ ac + 
  ...
  urban_sh + ownership_d + n_members + edu_head_2 + age_head + sex_head + housing_index_lab | adm1
...
ely_formula_ind <- ln_ely_q ~ ac +
  curr_CDD18_db + I(curr_CDD18_db^2) + 
  curr_HDD18_db + I(curr_HDD18_db^2) + ln_total_exp_usd_2011 + ln_ely_p +
  urban_sh + ownership_d + n_members + edu_head_2 + age_head + sex_head + housing_index_lab + selection | adm1
...
save(list = c("reg_ac", "HH_India", "model3", "dydx_ac"), 
     file = paste(interm,'ind_dmcf.RData', sep=''))
