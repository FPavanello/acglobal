...
ac_formula_bra <- ac ~ mean_CDD18_db + mean_CDD18_db2 + curr_CDD18_db + curr_CDD18_db2 +  
...
# With selection
model3 <- feols(ely_formula_bra, data = HH_Brazil, weights = ~weight, cluster = c("hhid")); summary(model3)

# Marginal effect of AC
ac_eff <- avg_slopes(model3, variables = "ac", slope = "dydx", wts = HH_Brazil$weight)
...
save(list = c("reg_ac", "HH_Brazil", "model3", "dydx_ac"), 
     file = paste(interm,'bra_dmcf.RData', sep=''))
