...
reg_ac <- svyglm(ac_formula_arg, design = HH_Argentina_svy,
...
ely_formula_arg  <- ln_ely_q ~ ac + ac*std_CDD + ac*I(std_CDD^2) + std_CDD + I(std_CDD^2) + 
  ...
  housing_index_lab + subregion + selection

# With selection
model <- svyglm(ely_formula_arg, design = HH_Argentina_svy, na.action=na.omit); summary(model)

# Marginal effect of AC
ely_margins <- summary(margins(model, design = HH_Argentina_svy))
