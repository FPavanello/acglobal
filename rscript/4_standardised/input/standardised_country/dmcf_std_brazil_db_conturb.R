...
ac_formula_bra <- ac ~ std_CDD_mean + I(std_CDD_mean^2) + std_CDD_mean*std_texp + I(std_CDD_mean^2)*std_texp + std_texp + std_CDD + I(std_CDD^2) +
  ...
  std_elyp*ownership_d + std_elyp*std_n_members + std_urban_sh + std_n_members + 
...
ac_margins <- avg_slopes(reg_ac, wts = HH_Brazil$weight)
gc()
...
ely_margins <- avg_slopes(model, slope = "dydx", wts = HH_Brazil$weight)

# Export
save(list = c("ely_margins", "ac_margins"), file = paste(interm,'standardised/bra_dmcf.RData', sep=''))
